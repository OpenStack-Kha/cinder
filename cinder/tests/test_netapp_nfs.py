...
class NetappDirectCmodeNfsDriverTestCase(test.TestCase):
    ...
    def test_register_img_in_cache_noshare(self):
        ...
        drv._get_provider_location('1').AndReturn('10.61.170.1:/share/path')
        drv._do_clone_rel_img_cache('testvol', 'img-cache-12345',
            '10.61.170.1:/share/path', 'img-cache-12345')
    ...
    def test_clone_image_fromcache(self):
        ...
        mox.StubOutWithMock(drv, '_do_clone_rel_img_cache')
        mox.StubOutWithMock(drv, '_post_img_clone')
        ...
        drv._post_img_clone(volume, '/mnt')

        mox.ReplayAll()
        drv. clone_image(volume, 'image_location', 'image_id')
        mox.VerifyAll()
    ...
    def test_clone_image_cloneableshare_nospace(self):
        ...
        (prop, cloned) = drv. clone_image(
            volume, 'nfs://127.0.0.1:/share/img-id', 'image_id')
        mox.VerifyAll()
    ...
    def test_clone_image_cloneableshare_raw(self):
        ...
        mox.StubOutWithMock(drv, '_resize_vol')
        mox.StubOutWithMock(drv, '_is_share_eligible')
        ...
        drv._resize_vol(
            {'name': 'vol'}, '127.0.0.1:/share', delete_on_fail=True)

        mox.ReplayAll()
        drv. clone_image(volume, 'nfs://127.0.0.1:/share/img-id', 'image_id')
        mox.VerifyAll()
    ...
    def test_clone_image_cloneableshare_notraw(self):
        ...
        image_utils.qemu_img_info('/mnt/vol').AndReturn(
            self.get_img_info('raw'))
        drv._register_image_in_cache(IgnoreArg(), IgnoreArg(), IgnoreArg())
        drv._set_rw_permissions_for_all('/mnt/vol')
        drv._resize_vol(
            {'name': 'vol'}, '127.0.0.1:/share', delete_on_fail=True)
